  ...
  return (
    <div className="rounded-lg border border-[rgba(41,41,41,0.5)] bg-[#0d0d0d]/80 p-6 mb-5">
      <div className="flex items-center justify-between mb-5">
        <h3 className="text-white text-base font-semibold">Mapa de Calor - Cadastrados por Dia e Hora</h3>
        <button className="flex items-center gap-2 border border-[rgba(41,41,41,0.5)] bg-[#0d0d0d]/50 text-white px-3 py-1.5 rounded text-xs">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          <thead>
            <tr>
              <th className="text-left text-[#999] pb-2 pr-3 font-normal w-10" />
              {hours.map((h, i) => (
                <th key={i} className="text-center text-[#999] pb-2 font-normal min-w-[56px]">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {days.map((day, di) => (
              <tr key={day}>
                <td className="text-[#999] pr-3 py-1 font-medium">{day}</td>
                {hours.map((_, hi) => {
                  const val = heatmapData[di][hi]
                  return (
                    <td key={hi} className="py-1 px-1">
                      <div
                        className="w-full h-9 rounded flex items-center justify-center text-white text-xs font-semibold"
      ...
      <div className="flex items-center justify-end gap-3 mt-4">
        <span className="text-[#999] text-xs">Menos cadastros</span>
        ...
        <span className="text-[#999] text-xs">Mais cadastros</span>
      </div>
    </div>
  )
}
