...
export function MetricasSection({ visibleIds }: { visibleIds: string[] }) {
  ...
  const gridClass = visibleIds.length >= 2 ? 'lg:grid-cols-2' : ''
  ...
  return (
    <>
      <div className={`grid grid-cols-1 ${gridClass} gap-5 mb-5`}>
        ...
        {/* Métricas de Vendas */}
        {visibleIds.includes('metricas_vendas') && (
        ...
        {/* Intermediações */}
        {visibleIds.includes('intermediacoes_m') && (
