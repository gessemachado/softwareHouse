      ...
      <div className="p-5 flex flex-col sm:flex-row gap-4 flex-1">
        <div className="relative w-full sm:w-52 h-52 sm:shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <p className="text-[#999] text-xs">Não Atuou</p>
            <p className="text-white text-xl font-bold">{(100 - aprovPct).toFixed(1)}%</p>
          </div>
        </div>
          ...
          {prodSubItems.map(({ id, label, cur, prev, color }) => vis(id) && (
            <div key={label} className="rounded-lg border border-[#1a1a1a] bg-black px-3 py-2.5">
              <div className="flex items-center gap-1.5 mb-1.5">
                ...
                <span className="text-[#999] text-xs font-semibold tracking-widest uppercase">{label}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-white text-xs font-bold">{fmtK(cur)}</span>
                <div className="flex items-center gap-1.5">
                  <TrendBadge cur={cur} prev={prev} />
                  <span className="text-[#666] text-xs">vs {fmtK(prev)}</span>
                </div>
              </div>
            </div>
          ))}
